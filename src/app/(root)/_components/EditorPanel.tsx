...
const EditorPanel = () => {
    ...
    const {language,fontSize,theme,editor,setEditor,setFontSize} = useCodeEditorStore()
    const mounted = useMounted();

    useEffect(() => {
        const savedCode = localStorage.getItem(`editor-code-${language}`) || LANGUAGE_CONFIG[language].defaultCode;
    ...
    useEffect(() => {
        ...
        if(savedFontSize){
            editor.setValue(parseInt(savedFontSize));
        }
    }, [setFontSize]);

    const handleRefresh = ()=>{
        const savedCode = LANGUAGE_CONFIG[language].defaultCode;
